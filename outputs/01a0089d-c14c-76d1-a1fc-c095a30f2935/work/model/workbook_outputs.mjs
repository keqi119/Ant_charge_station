...
function buildScenariosChecksAndSources(workbook) {
  ...
  const targetParityTerms = Array.from({ length: 56 }, (_, index) => `MOD('城市分配'!$J$${6 + index},2)`);
  const stationIntegerTerms = Array.from({ length: 56 }, (_, index) => [
    `MOD('城市分配'!$L$${6 + index},1)`,
    `MOD('城市分配'!$M$${6 + index},1)`,
  ]).flat();
  const cityIntegerGate = `=IF(AND('核心假设'!$B$6>0,MOD('核心假设'!$B$6,1)=0,MOD('核心假设'!$B$6,2)=0),0,1)+COUNTIF('城市分配'!$J$6:$J$61,"<0")+SUM(${targetParityTerms.join(",")})+COUNTIF('城市分配'!$L$6:$M$61,"<0")+SUM(${stationIntegerTerms.join(",")})`;
  const residualAmountGates = Array.from({ length: BASE_COHORTS }, (_, index) => {
    const row = 5 + index;
    return `IF('融资租赁与资金缺口'!$J$${row}>'融资租赁与资金缺口'!$E$${row},1,0)`;
  });
  const financeInputGate = `=MAX(ABS(SUM('融资租赁与资金缺口'!$E$5:$E$16)-SUM('融资租赁与资金缺口'!$D$5:$D$16)*'核心假设'!$B$27),ABS(MIN('融资租赁与资金缺口'!$O$5:$O$16)),ABS(MAX('融资租赁与资金缺口'!$O$5:$O$16)))+IF(OR('核心假设'!$B$27=80%,'核心假设'!$B$27=90%,'核心假设'!$B$27=100%),0,1)+IF(OR('核心假设'!$B$28=18,'核心假设'!$B$28=24,'核心假设'!$B$28=36),0,1)+IF(OR('核心假设'!$B$29=6%,'核心假设'!$B$29=8%,'核心假设'!$B$29=10%,'核心假设'!$B$29=12%),0,1)+IF(OR('核心假设'!$B$30=0,'核心假设'!$B$30=1,'核心假设'!$B$30=2),0,1)+IF(AND('核心假设'!$B$32>=0,'核心假设'!$B$32<='核心假设'!$B$27),0,1)+SUM(${residualAmountGates.join(",")})+COUNTIF('融资租赁与资金缺口'!$K$5:$K$16,"<0")+COUNTIF('融资租赁与资金缺口'!$B$70:$BI$81,"<0")`;
  const checks = [
    ["新增枪数累计精确等于目标", "=SUM('36月运营模型'!$B$7:$BI$7)", "='核心假设'!$B$6", 0, "36月运营模型!B7:BI7", "投放期后新增为0"],
    ["城市枪数/站数非负且站数为整数", cityIntegerGate, "=0", 0, "核心假设!B6,城市分配!J6:M61", "目标枪数必须为正偶数；逐城目标非负偶数，2枪/4枪站数非负整数"],
    ...
    ["融资输入、残值与放款", financeInputGate, "=0", 0.01, "核心假设!B27:B32,融资租赁与资金缺口!E5:O16,70:81", "融资比例/期限/成本/延迟属于批准域；逐批残值不超过融资额且无负月租或负摊销"],
